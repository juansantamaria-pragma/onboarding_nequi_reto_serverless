...
module.exports = async function defaultHandler(event, context) {
  try {
    let traceID = event.RequestMessage && event.RequestMessage.RequestHeader
      ? event.RequestMessage.RequestHeader.MessageID
      : undefined
    event.traceID = traceID
    lambdaUtils.log(`reto_serverless entrada [trace:${traceID}]`, event)
